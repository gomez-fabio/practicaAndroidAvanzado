...
import es.fabiogomez.madridshops.Activity.*
...
class Router {
 ...
 /*   fun navigateFromMainActivityToXXXXActivity(main: ShopsActivity){
    ...
    fun navigateFromActivitiesActivityToActivitiesDetailActivity(activity: ActivitiesActivity, mad_activity: Activity){
        val intent = Intent(activity, ActivityDetailActivity::class.java)

        intent.putExtra(INTENT_ACTIVITY_DETAIL_ACTIVITY, mad_activity)
        activity.startActivity(intent)
    }
